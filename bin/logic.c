...
int check(int board[8][8][2], int sx, int sy, int ex, int ey, const int king[2]) {
    ...
    // checks that it is a legal move, that it isn't staying on the same square, and that it isn't attacking a teammate
...
int eval(int board[8][8][2], int sx, int sy, int ex, int ey) {
    ...
    switch(p_type) {
        ...
        case 1:
            // makes sure that the sum of the distances is 3 and that one distance is 1 (the other has to be 2)
            return (abs(xdist) + abs(ydist) == 3) && (abs(xdist) == 1 || abs(ydist) == 1);
        case 2:
            return check_bishop(board, sx, sy, xdist, ydist);
        case 3:
            return check_queen(board, sx, sy, ex, ey, xdist, ydist);
        case 4:
            // makes sure that the king is only moving one space away
            return (abs(xdist) < 2 && abs(ydist) < 2);
        case 5:
...
int check_pawn(int board[8][8][2], int sx, int sy, int ex, int ey, int xdist, int ydist) {
    ...
    if (((
            ((xdist == 0 && abs(ydist) == 1) || (xdist == 0 && abs(ydist) == 2 && (sy == 1 || sy == 6))) && board[ey][ex][0] == -1) ||
            ((abs(xdist) == 1 && abs(ydist) == 1) && board[ey][ex][0] != -1)) &&
